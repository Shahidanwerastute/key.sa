...
@section('content')
        ...
        <div class="container-md <?php echo custom::addClass(); ?>">
            ...
            <div class="bookingStepsLink">
                ...
                <?php
                    ...
                    $from_cit = $sessionVals['from_city_id'];
                    $to_cit = $sessionVals['to_city_id'];
                    $pick_date = $sessionVals['pickup_date']?custom::getDateFormat($sessionVals['pickup_date'],$lang):'';
                    $drop_date = $sessionVals['dropoff_date']?custom::getDateFormat($sessionVals['dropoff_date'],$lang):'';
                    ...
                    custom::branch_mob_divs(false,$lang,$from_br,$to_br,$from_cit,$to_cit,$pick_date,$drop_date,$pickup_time,$dropoff_time,$lang_base_url,$edit, $is_delivery_mode);
                 ?>
            <div class="shotingLink">
                <a href="javascript:void(0);" data-bs-toggle="collapse" data-bs-target="#mob-menus"><?php echo ($lang == 'eng' ? 'Price Filtering' : 'ترتيب بالسعر'); ?></a>
                <div id="mob-menus" class="wrap-dropdown collapse">
                    <select name="" class="catId_mob_search" onchange="main_search_with_filter($(this).val());">
                        <option value="0" selected><?php echo($lang == 'eng' ? 'ALL' : 'الكل'); ?></option>
                        <?php foreach ($categories as $category){ ?>
                            <option value="<?php echo $category->id; ?>"><?php echo($lang == 'eng' ? $category->eng_title : $category->arb_title); ?></option>
                        <?php } ?>
                    </select>
                    <select onchange="main_search_with_filter($(this).val());">
                        <option disabled selected><?php echo ($lang == 'eng' ? 'Price Filtering' : 'ترتيب بالسعر'); ?></option>
    ...
    <section class="bookingSec">
        <div class="container-md">
            <?php if ($pickup_time_is_ok == true && $dropoff_time_is_ok == true && $no_of_hours_are_fine == true && $difference_is_fine == true && $pickup_dropoff_are_ahead_of_current_time == true && $delivery_slots_are_ok == true && $monthly_date_diff_is_fine == true && $weekly_date_diff_is_fine == true){ ?>
            <div class="containsData searchLoadMorePage">
                <?php if ($cars){
                    echo '<script>
