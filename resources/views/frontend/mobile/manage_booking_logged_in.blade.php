...
@section('content')
    <style>
        ...
        .totalsArea {
            ...
            padding-top: 10px;
            padding-bottom: 10px;
            border-bottom: 1px solid #EFA214;
        }
        .totalsArea p {
            margin-bottom: 0;
        }
        body.is_mobile.arb .pricePageSec .pricePgWrapper .carSumery .bookName {
            width:100%;
        }
        body.is_mobile.arb .pricePageSec .totalWdisValu.add p {
            line-height: 2;
        }
        body.is_mobile .pricePageSec .pricePgWrapper .carSumery .bookName {
            width: 92%;
        }
    </style>
        ...
        <div class="pricePgWrapper ">
            <div class="leftCol">
                <div class="reservation-message">
                    <div class="heading-bar">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                ?>
                                <li>
                                    <a id="bCancelBtn_<?php echo $booking_detail->id; ?>" href="javascript:void(0);" onclick="cancelBooking(<?php echo $booking_detail->id; ?>);">
                                        <?php echo($lang == 'eng' ? 'Cancel Booking' : 'إلغاء الحجز'); ?>
                                    </a>
                                </li>
                                <?php } ?>
                                    ...
                                    <li>
                                        <a class="bCancelBtn bEditBtn" data-bid="{{custom::encode_with_jwt($booking_detail->id)}}" href="<?php echo $lang_base_url . "/edit-booking/" . custom::encode_with_jwt($booking_detail->id);?>">@lang('labels.edit_booking')</a>
                                    </li>
                                    <?php } ?>
                                    ...
                                    <li>
                                        <a href="<?php echo $lang_base_url.'/add-payment?s=1&q='.custom::encode_with_jwt($booking_detail->id) ?>">
                                            <?php echo ($lang == 'eng' ? 'Add Payment' : 'إضافة دفعة'); ?>
                                        </a>
                                    </li>
                                    <?php } ?>
                    ...
                    <div class="booking-detail">
                        <h4>
                            <strong> <?php echo '#'.$booking_detail->reservation_code; ?> </strong>
                        </h4>
                        <span id="statusMsg"><?php echo $status_text; ?></span>
                    </div>
                </div>
                <style>
                    .col-12.status {
                ...
                if ($delivery_booking_status) {
                    echo '<div class="col-12 status">';
                    echo ($lang == 'eng' ? 'Delivery Status' : 'حالة التسليم') . ' <span class="d-block"> ' . $delivery_booking_status . ' </span>';
                    echo '</div>';
                }
                ?>
                ...
                <div class="totalWdisValu add peddLftSet <?php echo custom::hide_only_for_limousine($booking_detail->is_limousine); ?>">
                    <div class="bCenter">
                        ...
                        <p><span><?php echo number_format($total_rent_for_all_days, 2); ?>
                            ...
                            <?php echo $excludingExtras; ?>
                        </p>
                    </div>
                </div>
                <div class="col subTtl yellowBox totalsArea <?php echo custom::hide_only_for_limousine($booking_detail->is_limousine); ?>">
                    <div class="bCenter">
            ...
            <div class="rightCol">
                <div class="btn-holder">
                    <?php if (custom::show_add_payment_option($booking_detail->id)) { ?>
                        <a class="printBtn addPaymentBtn" href="<?php echo $lang_base_url.'/add-payment?s=1&q='.custom::encode_with_jwt($booking_detail->id) ?>">
                            <?php echo ($lang == 'eng' ? '<span>+</span> Extend Contract & Add Payment' : '<span>+</span> تمديد العقد او اضافة دفعة'); ?>
                        </a>
                    <?php } ?>
                    <a class="printBtn" href="<?php echo $lang_base_url . '/print-booking/'.custom::encode_with_jwt($booking_detail->reservation_code.'||EDxjrybEuppO'); ?>" target="_blank">
                        <?php echo($lang == 'eng' ? 'Print' : 'طباعة'); ?>
                    </a>
                <?php
                    $site = custom::site_settings();
                    $post_booking_hours_from_db = $site->post_booking_cancellation_hours.' hours';
                    ...
                    $cancel_date = new \DateTime($time);
                    $humanless_date = new \DateTime($time);
                    if($site->human_less_mode == 'on' && $booking_detail->human_less_state == 'Car Taken' && $booking_detail->booking_status == "Picked" && ($pick_up_date->getTimestamp() > $humanless_date->getTimestamp())){ ?>
                    <input type="hidden" id="booking_id" value="<?php echo $booking_detail->id; ?>">
                    <input type="hidden" id="vehicleID" value="<?php echo $booking_detail->vehicle_id; ?>">
                    <a href="javascript:void(0);" class="unlockIco btnUnlockCar">@lang("labels.btn_unlock_car")</a>
                <?php } ?>
                </div>
                ...
                if($site->human_less_mode == 'on' && $booking_detail->human_less_state == 'Acknowledged'){
                    ?>
                    <div class="section-hl">
                        <form method="get" class="getYourCarForm" action="<?php echo $lang_base_url; ?>/get-car/<?php echo base64_encode($booking_detail->id); ?>" >
                            ...
                            <div class="getCar" >
                                <input type="submit" class="btn_getCar" value="@lang('labels.btn_get_your_car')" >
                            </div>
                        </form>
                    </div>
                <?php }
                /*if($site->human_less_mode == 'on' && $site->hl_close_contract == 'on' && $booking_detail->human_less_state == 'Car Taken' && $booking_detail->booking_status == "Picked" && ($pick_up_date->getTimestamp() > $humanless_date->getTimestamp())){*/
                if($site->human_less_mode == 'on' && $site->hl_close_contract == 'on' && $booking_detail->human_less_state == 'Car Taken' && $booking_detail->booking_status == "Picked"){
                    ?>
                <div class="section-hl">
...
<!-- Modal -->
<div class="modal-mobile modal fade" id="confirmDel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-mobile modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-heading">
                <h4 class="modal-title" id="myModalLabel">
                    <?php if ($lang == "eng") echo "Confirm"; else echo "تؤكد"; ?>
                </h4>
            </div>
            <div class="modal-body confmDelfrm text-center">
                ...
                <form action="javascript:void(0);" method="get">
                    ...
                    <div class="twoBtnEd">
                        <input type="submit" class="redishButtonRound" value="<?php if ($lang == "eng") echo 'Yes'; else echo 'نعم فعلا'; ?>"/>
                        <input type="button" class="grayishButton" value="<?php if ($lang == "eng") echo 'No'; else echo 'لا'; ?>" data-bs-dismiss="modal"/>
                        <div class="clearfix"></div>
                    </div>
                </form>
            </div>
