...
<head>
    ...
    <title><?php custom::getSiteName($lang); ?></title>
    ...
    <link href="{{custom::baseurl('public/frontend/css/bootstrap.css')}}" rel="stylesheet" type="text/css"
          media="all">

    <link href="{{custom::baseurl('public/frontend/css/all.css?v=0.1')}}" rel="stylesheet" type="text/css" media="all">
    <?php if($lang == 'arb'){ ?>
    <link href="{{custom::baseurl('public/frontend/css/rtl.css')}}" rel="stylesheet" type="text/css" media="all">
    <?php } ?>
...
<body class="<?php echo $class ; ?>" style="direction:<?php echo $direction; ?>; margin:0; padding:0; font-family: 'Lato', sans-serif; font-size:13px; font-weight:400; line-height:16px; color:#858585; background:#fff;">
                	...
                	<tr>
                    	<td style="padding:14px 33px 14px 50px; background:#444; border-bottom:2px solid #f1af1d;" class="topbar">
                        	<table border="0" cellpadding="0" cellspacing="0" width="100%">
                            	<tr>
                                    <?php
                                    if($_SERVER['SERVER_NAME'] == 'key.sa' || $_SERVER['SERVER_NAME'] == 'key.ed.sa') {
                                        $phone_numb = custom::site_phone();
                                        $website = 'www.key.sa';
                                    }else{
                                        $phone_numb = '920004717';
                                        $website = 'www.awfar.sa';
                                    }

                                    ?>
									<?php $path_icon = custom::baseurl('/resources/views/frontend/emails/images/ico-tell.png'); ?>
                                	<td><img src="<?php if(isset($message)){ echo $message->embed($path_icon); } else{ echo $path_icon;} ?>" alt="" width="10" height="12" style="display:inline-block; margin:0; vertical-align:middle;">&nbsp;&nbsp;<a href="tel:<?php echo $phone_numb; ?>" style="color:#fff; font-family: 'Lato', sans-serif; font-size:12px; font-weight:400; cursor:pointer; text-decoration:none;"><?php echo $phone_numb; ?></a></td>
                                    <td style="text-align:left;"><a href="<?php echo custom::baseurl('/'); ?>" style="font-size:12px; color:#f8ac1d; font-family: 'Lato', sans-serif; text-decoration:none;"><?php echo $website; ?></a></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                    	<td class="regisNewUserPg " style=" line-height: 1.3; padding: 20px;">
                            <?php
                            if($lang == 'eng'){
                                echo 'Do you want to transfer the booking into the online website to get '.$walkin_discount_percent.'% discount';
                            }else{
                                echo 'هل تريد تأكيد نقل حجزك إلى حجز عن طريق الموقع للحصول على خصم '.$walkin_discount_percent.'%';
                            }
                            ?>
                            <br>
                            ...
                            <br>
                            <button type="button" class="redishButtonRound edBtn " onclick="confirmWalkin('yes');">@lang('labels.walkin_yes')</button>
                            <button type="button" class="grayishButton edBtn "  onclick="confirmWalkin('no');">@lang('labels.walkin_no')</button>
                    	</td>
                    </tr>
                    <tr>
                    	...
                    	<td style="border-top:2px solid #f8ab1d; background:#2a2521; padding:14px 36px 16px 52px;" class="tfoot">
                        	<table border="0" cellpadding="0" cellspacing="0" width="100%">
                            	<tr class="tr">
                                	<td style="color:#fff; font-size:9px; line-height:14px; font-weight:700; font-family: 'Lato', sans-serif;" class="col"><?php echo ($lang == 'eng' ? 'COPYRIGHTS' : 'حقوق الملكية'); ?> <?php echo date('Y'); ?>.<?php echo ($lang == 'eng' ? ' ALL RIGHTS RESERVED.' : 'جميع الحقوق محفوظة.'); ?>.</td>
                                    <td style="text-align:left;" class="col">
                                    	<table border="0" cellpadding="0" cellspacing="0" width="100%">
                                        	<tr>
                                            	<td style="font-size:8px; line-height:14px; padding-right:10px; color:#fff;"><?php echo ($lang == 'eng' ? 'FOLLOW  US' : 'تابعونا'); ?></td>
        ...
        <div class="modal-dialog bigOne" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="twoBtnEd">
                        <p>&nbsp;</p>
                        <input class="redishButtonRound" value="@lang('labels.close')"
    ...
    <form action="<?php echo $lang_base_url.'/saveWalkinBookingOnConfirm'; ?>" method="post" class="walkin_confirm_form">
        <input type="hidden" name="full_name" value="<?php echo $walkin_data->customer_first_name." ".$walkin_data->customer_last_name; ?>">
        <input type="hidden" name="mobile_no" value="<?php echo $walkin_data->customer_mobile; ?>">
        <input type="hidden" name="walkin_id" value="<?php echo $walkin_data->id; ?>">

    </form>

<script type="text/javascript">
    var doplication = false;
    function confirmWalkin(val){
        if(val=='no'){
            if(confirm("are you sure you want to ignore?"))
            {
                window.location.href = "<?php echo $lang_base_url; ?>"
            }
        }else if(val == 'yes'){
            if($("#iAccept").is(':checked')){
                if(!doplication) {
                    doplication = true;
                    $(".walkin_confirm_form").submit();
                }
            }else{
