@extends('admin.layouts.template')

@section('content')
    <?php //print_r(custom::api_settings());exit(); ?>
    <div id="page_content">
        <div id="page_content_inner">
            <div class="uk-grid" data-uk-grid-margin>
                <div class="uk-width-medium-1-1">
                    <div class="md-card">
                        <form action="<?php echo custom::baseurl('/'); ?>/admin/settings/save_terms_conditions" method="post" class="settings_ajax_form" onsubmit="return false;">
                            <div class="md-card-content">
                                <h3 class="heading_a">Terms & Conditions</h3>
                                <div class="uk-grid" data-uk-grid-margin>
                                    <div class="uk-width-medium-1-2">
                                        <label>English Content (For Website)</label>
                                        <div class="uk-form-row">
                                            <textarea cols="30" rows="8" class="md-input" name="eng_terms" id="terms_eng"><?php echo $site->eng_terms; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="uk-width-medium-1-2">
                                        <label>Arabic Content (For Website)</label>
                                        <div class="uk-form-row">
                                            <textarea cols="30" rows="8" class="md-input" name="arb_terms" id="terms_arb"><?php echo $site->arb_terms; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="uk-width-medium-1-2">
                                        <label>English Content (For CDW Plus)</label>
                                        <div class="uk-form-row">
                                            <textarea cols="30" rows="8" class="md-input" name="eng_terms_for_cdw_plus" id="terms_eng_for_cdw_plus"><?php echo $site->eng_terms_for_cdw_plus; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="uk-width-medium-1-2">
                                        <label>Arabic Content (For CDW Plus)</label>
                                        <div class="uk-form-row">
                                            <textarea cols="30" rows="8" class="md-input" name="arb_terms_for_cdw_plus" id="terms_arb_for_cdw_plus"><?php echo $site->arb_terms_for_cdw_plus; ?></textarea>
                                        </div>
                                    </div>
                                    <input type="hidden" name="id" value="<?php echo $site->id; ?>">
                                    <?php if (custom::rights(23, 'edit'))
                                    { ?>
                                    <div class="uk-width-1-1">
                                        <button type="submit" href="#" class="md-btn md-btn-primary">Update</button>
                                    </div>
                                    <?php } ?>
                                </div>
                            </div>
                        </form>
                    </div>

                </div>

            </div>
        </div>

    </div>
@endsection