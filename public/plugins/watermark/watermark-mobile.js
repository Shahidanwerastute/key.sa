...
(function($) {
    ...
    var destroyWatermarker = function(elements) {
        ...
        $elements.each(function() {
            ...
            if (data.length > 0) {
                ...
                $(document).off("touchstart", jQueryClassName(data[0].eventTriggerClass) + " " + jQueryClassName(data[0].watermarkerClass));
                $(document).off("touchstart", jQueryClassName(data[0].eventTriggerClass) + " " + jQueryClassName(data[0].resizerClass));
    ...
    $.watermarker = function(object, userOptions) {
        ...
        var watermark = $("<div>", {
            "class": watermarkerClassName,
            "data": userOptions.data
        }).css("left", options.offsetLeft).css("top", options.offsetTop);
        var watermarkImage = $("<img>", {
            src: options.imagePath,
            "class": watermarkImageClassName
        });
        var resizer = $("<div>", {
            "class": options.resizerClass
        });
        ...
        var dragEvent = function(event) {
            ...
            var leftDifference = event.originalEvent.touches[0].pageX - cssLeft;
            var cssTop = parseInt($(this).css("top"));
            var topDifference = event.originalEvent.touches[0].pageY - cssTop;
            //$(this).data("difX", parseInt( $(this).css("left") ) - event.originalEvent.touches[0].pageX + document.body.scrollTop);
            $(this).data("difX", leftDifference);
            $(this).data("difY", topDifference);
            //$(this).data("difY", parseInt( $(this).css("top") ) - event.originalEvent.touches[0].pageY + document.body.scrollTop);
        ...
        var resizeEvent = function(event) {
            ...
            resizer.data("startPositionX", event.originalEvent.touches[0].pageX);
            resizer.data("startPositionY", event.originalEvent.touches[0].pageY);
        ...
        var drag = function(event) {
            ...
            var container = element.parentNode;
            //var left = parseInt(event.originalEvent.touches[0].pageX) + $(element).data("difX") + "px";
            var left = parseInt(event.originalEvent.touches[0].pageX) - $(element).data("difX") + "px";
            //var top = parseInt(event.originalEvent.touches[0].pageY)  + $(element).data("difY") + "px";
            var top = parseInt(event.originalEvent.touches[0].pageY) - $(element).data("difY") + "px";
        ...
        var resize = function(event) {
            ...
            //var width = parseInt(event.originalEvent.touches[0].pageX) /*- $(options.container).offset().left */- parseInt(element.style.left.length > 0 ? element.style.left : 0 ) ;
            
			var width = resizerElement.data("startWidth") + event.originalEvent.touches[0].pageX - resizerElement.data("startPositionX");
            //var width = parseInt(event.originalEvent.touches[0].pageX) - $(options.container).offset().left - $(element).position().left ;
            var height;
            if (options.aspectRatio !== undefined && options.aspectRatio !== null) {
                height = width / options.aspectRatio;
            } else {
                //height = parseInt(event.originalEvent.touches[0].pageY) /*- $(options.container).offset().top*/ - parseInt(element.style.top.length > 0 ? element.style.top : 0);	
                height = resizerElement.data("startHeight") + event.originalEvent.touches[0].pageY - resizerElement.data("startPositionY");
            }
			//console.log(width);
        ...
        if (isFirstWatermark(data)) {
            if(draggingClass()) $(document).on("touchstart", jQueryClassName(options.eventTriggerClass) + " " + jQueryClassName(options.watermarkerClass), dragEvent);
        ...
        if (options.allowRemove) {
            ...
            var $removeContainer = $("<div>", {
                "class": options.removeClass
            });
            var $img = $("<img>", {
                src: options.removeIconPath
            });
            $removeContainer.append($img);
            $(watermark).append($removeContainer);
            $(watermark).on("click touchstart", "." + getClassName(options.removeClass), function(event) {
