    ...
    $.fn.kitkatclock=function(options){
        return this.each(function(){
            ...
            var input=$(this).on("mouseup", function(){
				...
				var gp = getPos(document.getElementsByClassName('pickUpTime')[0]);
				var mainSearchBT = gp.y;
				//alert(mainSearchBT);
				//return false;
                var plugin_position={
            ...
            function init_clock(){
                ...
                am_pm_text=$("<div class='amPmTop'>").css({
                ...
                am_pm.trigger("change");
                container.addClass('clockEdShow');
                container.css({
                    width:clock_dim, height:clock_dim+(fontSize*2)+10,
                    zIndex:9999999,
                    backgroundColor:colors.background,
                    borderRadius:"0", padding:"3px "+padding+"px",
                    fontFamily: fontFamily, color:colors.text
                });
                canvas_container=$("<div class='canContEd'>").css({
                    position:"absolute",
                ...
                var time_display=$("<div class='timeTopEd'>");
                ...
                var done_button=$("<div class='clockDone'>Done</div>").css({
                ...
                canvas_container.on("mousemove mousedown mouseup touchstart touchend touchcancel click touchmove", function(event){
                    ...
                    if (use_event=="click" ||
                        ...
                        //kashif
                        if(!hour_mode) {
                            if (number > 0 && number <= 15) number = 15;
                            if (number > 15 && number <= 30) number = 30;
                            if (number > 30 && number <= 45) number = 45;
            ...
            function draw_hand_orig(){
                ...
                ctx.clearRect(0, 0, clock_dim, clock_dim);
                var a_x=radius+(Math.cos(radians)*(-1)*(radius-fontSize/1.1));
                var a_y=radius+(Math.sin(radians)*(-1)*(radius-fontSize/1.1));
                ctx.beginPath();
                ...
                ctx.stroke();
                ctx.closePath();
                ctx.arc(a_x, a_y, fontSize*(1.40/1.5), 0 , 3*pi);
