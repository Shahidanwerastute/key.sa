...
class ServicesController extends Controller
{
    ...
    public function getCars()
    {
        try {
            ...
            if ($site_settings->weekly_renting_mode_mobile == 'on' && $is_delivery_mode == 5) {
                $date_picked_up_for_weekly_cal = new Carbon($_REQUEST['pickup_date'] . ' ' . $pickup_time);
                $date_dropped_off_for_weekly_cal = new Carbon($_REQUEST['dropoff_date'] . ' ' . $dropoff_time);
                $months_diff = $date_dropped_off_for_weekly_cal->diffInDays($date_picked_up_for_weekly_cal);
                if ($months_diff < 7) {
                    $weekly_date_diff_is_fine = false;
                    $expected_dropoff_date = $date_picked_up_for_weekly_cal->addWeek(1)->toDayDateTimeString();
                    $weekly_date_diff_is_fine_msg = ($this->lang == 'eng' ? 'The dropoff date must be a week from pickup date.' : 'تاريخ التسليم يجب ان يكون شهر من تاريخ الاستلام');
                }
            }

            if ($no_of_hours_are_fine == false || $difference_is_fine == false || $pickup_time_is_ok == false || $dropoff_time_is_ok == false || $pickup_dropoff_are_ahead_of_current_time == false || $delivery_slots_are_ok == false || $monthly_date_diff_is_fine == false || $weekly_date_diff_is_fine == false) {
                ...
                } elseif ($weekly_date_diff_is_fine == false) {
                    $errorMsg = $weekly_date_diff_is_fine_msg;
                }
                $response['status'] = 0;
                ...
                echo json_encode($response, JSON_UNESCAPED_UNICODE);
                exit();
            } elseif ($no_of_hours_are_fine == true && $difference_is_fine == true && $pickup_time_is_ok == true && $dropoff_time_is_ok == true && $pickup_dropoff_are_ahead_of_current_time == true && $delivery_slots_are_ok == true && $monthly_date_diff_is_fine == true && $weekly_date_diff_is_fine == true) {
                ...
                if ($cars) {
                    ...
                    $query = "SELECT * FROM setting_renting_type WHERE from_days <= '" . $search_by['days'] . "' order by from_days desc limit 1";
                    ...
                    foreach ($cars as $car) {
                        ...
                        if ($booking_per_day_found !== false) {
                            if ($booking_per_day_found) {
                                ...
                                if ($booking_per_day_found_bookings < $booking_per_day_found) {
                                    ...
                                    if ($site_settings->car_utilization_mode_mobile == 'on' && false) {
                                    ...
                                    if (strtolower($customer_type) == 'individual' && $renting_type_details->oracle_reference_number == '26757' && $is_delivery_mode == 0 && $site_settings->car_utilization_addition_subtraction_mode == 'on') {
                                        ...
                                        if ($days_to_check_for_car_utilization_addition_subtraction_mode > 0) {
                                            ...
                                            $date2 = Carbon::parse($_REQUEST['pickup_date']);

                                            if ($date1->diffInDays($date2) < $days_to_check_for_car_utilization_addition_subtraction_mode) {

                                                $from_branch_details = $page->getSingle('branch', ['id' => $_REQUEST['branch_id_from']]);
                                                $car_model_details = $page->getSingle('car_model', ['id' => $car->car_id]);
                                                ...
                                                if ($car_utilization_setup && $car_utilization_setup->addition_or_subtraction_percentage != 0) {
                                                    $car->discounted_price = $car->discounted_price + (($car_utilization_setup->addition_or_subtraction_percentage / 100) * $car->discounted_price);
                                                    $car->actual_price = $car->actual_price + (($car_utilization_setup->addition_or_subtraction_percentage / 100) * $car->actual_price);
                        ...
                        $cars[$i]->is_for_disabled = $car->is_for_disabled;
            ...
            if (!$checkIfRecordExist) {
                ...
                if ($search_query == '') {
                    ...
                    if (
                        ...
                        $booking_detail['delivery_booking_status'] = custom::delivery_booking_statuses($booking_detail['delivery_booking_status'], $this->lang);
    ...
    public function getDropdownsDataForRegistration()
    {
        try {
            ...
            $weekly_renting_mode = ($site_settings->weekly_renting_mode_mobile == 'off' ? 0 : 1);
            ...
            $weekly_with_delivery_flow = ($site_settings->weekly_with_delivery_flow == 'off' ? 0 : 1);
            ...
            $data['weekly_renting_mode'] = (int)$weekly_renting_mode;
            ...
            $data['weekly_tab_title_eng'] = $site_settings->weekly_tab_title_eng;
            $data['weekly_tab_title_arb'] = $site_settings->weekly_tab_title_arb;
            ...
            $loyalty_programs = $this->page->getMultipleRows('setting_loyalty_programs', array('is_active' => 1), 'id', 'asc');
            ...
            $data['weekly_with_delivery_flow'] = $weekly_with_delivery_flow;
        ...
        try {
            ...
            if (count($bookings) > 0) {
                ...
                foreach($bookings as $booking) {
                    ...
                    $booking->print_url = $this->lang_base_url . '/print-booking/' . custom::encode_with_jwt($booking->print_url);
                }
    ...
    public function book()
    {
        try {
            ...
            $booking_payment_data['car_rate_is_with_additional_utilization_rate'] = (isset($_REQUEST['car_rate_is_with_additional_utilization_rate']) && $_REQUEST['car_rate_is_with_additional_utilization_rate'] ? $_REQUEST['car_rate_is_with_additional_utilization_rate'] : 0);
...
');
    ...
    public function get_home_slider_images(Request $request) {
        ...
        foreach ($slider_images as $slider_image) {
            ...
            $url = '';
            if ($this->lang == 'eng' && $slider_image->eng_url) {
                $url = $slider_image->eng_url;
            } elseif ($this->lang == 'arb' && $slider_image->arb_url) {
                $url = $slider_image->arb_url;
            }

            $slider_images_array[$i]['image'] = custom::baseUrl('public/uploads/' . ($this->lang == 'eng' ? $slider_image->image1 : $slider_image->image2));
            $slider_images_array[$i]['url'] = $url;
            $i++;
        }
        $response['status'] = 1;
        $response['message'] = "Success";
